...
def merge_sorted(xs, cmp=cmp_standard):
    ...
    if len(xs) <= 1:
        ...
        left = xs[:mid]
        right = xs[mid:]
        left_sort = merge_sorted(left, cmp=cmp)
        right_sort = merge_sorted(right, cmp=cmp)
        return _merged(left_sort, right_sort, cmp=cmp)
...
def quick_sorted(xs, cmp=cmp_standard):
    ...
    if len(xs) <= 1:
        ...
        xs_smaller_s = quick_sorted(xs_smaller, cmp=cmp)
        xs_bigger_s = quick_sorted(xs_bigger, cmp=cmp)
        return xs_smaller_s + xs_equal + xs_bigger_s
...
def quick_sort(xs, cmp=cmp_standard):
    ...
    longer be a [stable sort](https://en.wikipedia.org/wiki/Sorting_
    algorithm#Stability),
...
'''
    def lomuto(xs, lo = 0, hi = 0):
        if lo >= xs[hi] or xs[lo] < 0:
            return xs
        pivot = xs[hi]
        i = lo - 1
